...
use sqlx::{Arguments, FromRow, MySql, Row, Transaction};

use super::traits::MySqlDbPool;
...
fn build_arguments(param_names: &[String], json_value: &Value) -> Result<MySqlArguments> {
...
fn add_json_value_to_args(args: &mut MySqlArguments, value: &Value) -> Result<()> {
    match value {
        Value::Null => {
