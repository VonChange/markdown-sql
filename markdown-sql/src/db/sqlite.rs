...
use crate::typed_params::TypedParamsSqlite;
...
where
    T: for<'r> FromRow<'r, SqliteRow> + Send + Unpin,
    P: Serialize + TypedParamsSqlite,
    D: DbPool,
{
    let (sql, param_names, _json_value) = prepare_sql(manager, sql_id, params)?;

    let mut args = SqliteArguments::default();
    params.bind_to_sqlite_args(&param_names, &mut args)?;
...
/// 查询单条（SQLite，可选，类型感知，事务）
pub async fn query_optional_typed_tx<'t, T, P>(
    manager: &SqlManager,
    tx: &mut Transaction<'t, Sqlite>,
    sql_id: &str,
    params: &P,
...
/// 查询单条（SQLite，必须存在，类型感知，事务）
pub async fn query_one_typed_tx<'t, T, P>(
    manager: &SqlManager,
    tx: &mut Transaction<'t, Sqlite>,
    sql_id: &str,
    params: &P,
...
/// 查询标量值（SQLite，类型感知，事务）
pub async fn query_scalar_typed_tx<'t, P>(
    manager: &SqlManager,
    tx: &mut Transaction<'t, Sqlite>,
    sql_id: &str,
    params: &P,
