//! PostgreSQL 集成测试
//!
//! 需要本地运行 PostgreSQL 容器：
//! docker run -d --name postgres-test -e POSTGRES_PASSWORD=root123 -e POSTGRES_DB=testdb -p 5432:5432 postgres:15
//!
...
/// PostgreSQL 测试 Repository
///
/// 使用 db_type = "postgres" 指定数据库类型
#[repository(sql_file = "tests/sql/PgTestRepository.md", db_type = "postgres")]
pub trait PgTestRepository {
    /// 删除表
...
static SQL_MANAGER: Lazy<SqlManager> = Lazy::new(|| {
    ...
    manager
        .load_file("tests/sql/PgTestRepository.md")
        .expect("加载 SQL 文件失败");
...
/// 获取 Repository 实例
fn get_repo() -> PgTestRepositoryImpl {
    PgTestRepositoryImpl::new(&SQL_MANAGER)
}
...
#[tokio::test]
async fn test_pg_insert_and_query() {
    let db = match setup_database().await {
        Some(x) => x,
        None => {
            println!("⚠️ PostgreSQL 未运行，跳过测试");
            return;
        }
    };
    ...
    assert_eq!(users[0].name, "PG测试用户");

    cleanup(&db).await;
    println!("✅ PostgreSQL 插入和查询测试通过");
}
